...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="调用 /api/v1/index/build_markdown 接口，从本地 md 目录构建知识库",
    )
    parser.add_argument(
        "--input-dir",
        type=str,
        default=r"D:\project\company_product\ICAUS2025-paper_md_cn",
        help="MD 文件所在目录",
    )
    parser.add_argument(
        "--kb-id",
    ...
    parser.add_argument(
        "--api-url",
        type=str,
        default="http://192.168.1.5:5010/api/v1/index/build_markdown",
        help="索引构建接口地址",
    )
    ...
    json_files = sorted(input_dir.glob("*.md"))
    if not json_files:
